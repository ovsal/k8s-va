...
#### Увеличение размера тома (PVC) для приложений

У StorageClass **`longhorn-ha`** в манифесте включено **`allowVolumeExpansion: true`**, поэтому **увеличить** запрошенный размер уже существующего PVC можно без пересоздания тома с нуля.

**Важно:** уменьшить (`shrink`) размер PVC в Kubernetes **нельзя** — только увеличение вверх или новый том с миграцией данных.

**Типовой порядок действий**

1. Убедиться, что PVC использует Longhorn и нужный класс:
   ```bash
   kubectl get pvc -n <namespace> <имя-pvc> -o wide
   ```
2. Запросить новый размер (пример — **50Gi**):
   ```bash
   kubectl -n <namespace> patch pvc <имя-pvc> --type merge \
     -p '{"spec":{"resources":{"requests":{"storage":"50Gi"}}}}'
   ```
3. Проверить события и условия тома (Longhorn и kubelet догоняют resize асинхронно):
   ```bash
   kubectl -n <namespace> describe pvc <имя-pvc>
   ```
   В Longhorn UI (**Volume**) можно увидеть состояние тома и реплик.

4. **Файловая система внутри пода:** для большинства сценариев после успешного расширения блочного устройства достаточно, чтобы под с томом **перезапустился** (или сработал онлайн-resize, если драйвер и ФС это поддерживают в вашей связке). Если в `describe pod` видно, что размер блока уже новый, а `df` внутри контейнера — старый, перезапустите StatefulSet/Deployment или сам под.

**Обычный Deployment / PVC из манифеста:** после `patch` PVC обновите в Git запись **`resources.requests.storage`**, чтобы декларативная конфигурация совпадала с кластером.

**StatefulSet и Helm (например Loki):** поле **`spec.volumeClaimTemplates`** у StatefulSet **нельзя изменить** после создания — смена размера только в Helm values **не перезапишет** шаблон PVC в API. Реальное расширение — снова через **`kubectl patch pvc`** на уже созданный claim; нюансы GitOps для Loki описаны в **`docs/observability.md`** (раздел про Loki и диск).
