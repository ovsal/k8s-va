...
```bash
ssh ansible@<NEW_NODE_IP>
```

### 2. Добавить ноду в inventory
В `cluster/inventory/prod/hosts.yaml`:
```yaml
worker-N:
...
```bash
cd cluster && ansible-playbook playbooks/00-host-prep.yaml \
  --limit worker-N
```

### 4. Добавить ноду через Kubespray scale
```bash
cd cluster && ansible-playbook kubespray/scale.yml \
  --limit worker-N
```
